...
export const isModifier = (key) => MODIFIER_KEYS.includes(key);

// A valid key is a single char (\w covers a-z0-9_, plus the symbol class) or a named key.
const VALID_KEY =
  /^([\w`~!@#$%^&*()\-=+[{}\]\\|;:'",<.>/?]|space|tab|enter|backspace|delete|escape|esc|arrow(up|down|left|right)|f[1-9]|f1[0-2]|home|end|page(up|down))$/;
...
export const parseInput = (input) => {
  ...
  const normalized = input
    .toLowerCase()
    .replace(/(?<=(ctrl|cmd|shift|alt|super))-/g, "+");
...
const MODIFIER_KEYS = ["control", "shift", "alt", "meta", "ctrl", "cmd"];
