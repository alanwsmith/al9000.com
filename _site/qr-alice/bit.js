const rawText = `Alice's Adventures in Wonderland
...
export function init(_, __, el) {
  running = false;
  el.replaceChildren(b.render("initState"));
}

export function start(_, __, el) {
  running = true;
  el.replaceChildren(b.render("runningState"));
  qrcode = new QRCode(
  ...
  text = rawText.split(/\s+/g);
  b.trigger("showWord");
}

export function pause(_, __, el) {
  running = false;
  el.innerHTML = "start";
  el.dataset.s = "start";
}
...
export async function showWord() {
  if (running) {
    ...
    qrcode.clear();
    qrcode.makeCode(
      `https://al9000.com/qr-alice/word/index.html?word=${text[textIndex]}`,
    );
    await b.sleep(80);
    b.trigger("showWord");
  }
