...
export const b = { init: "loadWords" };

let currentWord = 0;
let words = [];

export async function loadWords(_, __, el) {
  const result = await b.getTemplates("/qr-story/text.html");
  if (result) {
    words = b.text.text.split(/\s+/g);
    words.forEach((word, wordIndex) => {
      const id = `word${wordIndex}`;
      const subs = {
        __ID__: id,
      };
      el.appendChild(b.render("wordTemplate", subs));
    });
  }
  loadChunk();
}
...
export async function loadChunk() {
  const count = Math.min(400, words.length - currentWord);
  if (count > 0) {
    for (let i = 0; i <= count - 1; i += 1) {
      const el = document.getElementById(`word${currentWord}`);
      el.replaceChildren();
      new QRCode(
        el,
        {
          text: `https://al9000.com/qr-story/word/index.html?word=${
            words[currentWord]
          }`,
          width: 45,
          height: 45,
          colorLight: randomHex(),
        },
      );
      currentWord += 1;
    }
